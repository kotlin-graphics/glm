...
class funcExponential {
    ...
    fun exp() {
        ...
        val a = 1f.exp()
        assert(a.equal(glm.epsilon.f, 0.01f))

        val b = Vec1(1f).exp()
        assert(b.allEqual(Vec1(glm.epsilon.f), 0.01f))

        val c = Vec2(1f).exp()
        assert(c.allEqual(Vec2(glm.epsilon.f), 0.01f))

        val d = Vec3(1f).exp()
        assert(d.allEqual(Vec3(glm.epsilon.f), 0.01f))

        val e = Vec4(1f).exp()
        assert(e.allEqual(Vec4(glm.epsilon.f), 0.01f))
    }

    @Test
    fun log() {

        val a = glm.epsilon.f.log()
        assert(a.equal(1f, 0.01f))

        val b = Vec1(glm.epsilon.f).log()
        assert(b.allEqual(Vec1(1f), 0.01f))

        val c = Vec2(glm.epsilon.f).log()
        assert(c.allEqual(Vec2(1f), 0.01f))

        val d = Vec3(glm.epsilon.f).log()
        assert(d.allEqual(Vec3(1f), 0.01f))

        val e = Vec4(glm.epsilon.f).log()
