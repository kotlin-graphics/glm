...
abstract class Vec2t<T : Number>(_x: T, _y: T) : Vector2<T> {
    ...
    infix operator fun invoke(v: Vec2t<out Number>) = put(v.x, v.y)
    infix operator fun invoke(v: Vec3t<out Number>) = put(v.x, v.y)
    infix operator fun invoke(v: Vec4t<out Number>) = put(v.x, v.y)

    infix operator fun invoke(v: Vec2bool) = put(v.x.b, v.y.b)
    infix operator fun invoke(v: Vec3bool) = put(v.x.b, v.y.b)
    infix operator fun invoke(v: Vec4bool) = put(v.x.b, v.y.b)
    ...
    infix operator fun invoke(s: Number) = put(s, s)

    // -- indexed Generic Constructors --

    operator fun invoke(bytes: ByteArray, index: Int) = put(bytes[index], bytes[index + 1])
    operator fun invoke(chars: CharArray, index: Int) = put(chars[index].b, chars[index + 1].b)
    operator fun invoke(shorts: ShortArray, index: Int) = put(shorts[index], shorts[index + 1])
    operator fun invoke(ints: IntArray, index: Int) = put(ints[index], ints[index + 1])
    operator fun invoke(longs: LongArray, index: Int) = put(longs[index], longs[index + 1])
    operator fun invoke(floats: FloatArray, index: Int) = put(floats[index], floats[index + 1])
    operator fun invoke(doubles: DoubleArray, index: Int) = put(doubles[index], doubles[index + 1])
    operator fun invoke(booleans: BooleanArray, index: Int) = put(booleans[index].b, booleans[index + 1].b)

    operator fun invoke(a: Array<out Number>, index: Int) = put(a[index], a[index + 1])
    operator fun invoke(a: Array<Char>, index: Int) = put(a[index].b, a[index + 1].b)
    operator fun invoke(a: Array<Boolean>, index: Int) = put(a[index].b, a[index + 1].b)
    ...
    operator fun invoke(bytes: ByteBuffer, index: Int) = put(bytes[index], bytes[index + 1])
    operator fun invoke(chars: CharBuffer, index: Int) = put(chars[index].b, chars[index + 1].b)
    operator fun invoke(shorts: ShortBuffer, index: Int) = put(shorts[index], shorts[index + 1])
    operator fun invoke(ints: IntBuffer, index: Int) = put(ints[index], ints[index + 1])
    operator fun invoke(longs: LongBuffer, index: Int) = put(longs[index], longs[index + 1])
    operator fun invoke(floats: FloatBuffer, index: Int) = put(floats[index], floats[index + 1])
    operator fun invoke(doubles: DoubleBuffer, index: Int) = put(doubles[index], doubles[index + 1])
