...
import  mat.Mat2x3
import  mat.Mat3x3
import  mat.Mat3x3.Companion.add
import  mat.Mat3x3.Companion.div
import  mat.Mat3x3.Companion.mul
import  mat.Mat3x3.Companion.sub
import  mat.Mat4x3
import  vec._3.Vec3
...
interface mat3x3_operators {
    ...
    fun add(res: Mat3x3, a: Mat3x3, b: Float): Mat3x3 {
        add(res[0], a[0], b, b, b)
        add(res[1], a[1], b, b, b)
        add(res[2], a[2], b, b, b)
        return res
    ...
    fun add(res: Mat3x3, a: Mat3x3, b: Mat3x3): Mat3x3 {
        add(res[0], a[0], b[0][0], b[0][1], b[0][2])
        add(res[1], a[1], b[1][0], b[1][1], b[1][2])
        add(res[2], a[2], b[2][0], b[2][1], b[2][2])
        return res
    }


    fun sub(res: Mat3x3, a: Mat3x3, b: Float): Mat3x3 {
        sub(res[0], a[0], b, b, b)
        sub(res[1], a[1], b, b, b)
        sub(res[2], a[2], b, b, b)
        return res
    ...
    fun sub(res: Mat3x3, a: Float, b: Mat3x3): Mat3x3 {
        sub(res[0], a, a, a, b[0])
        sub(res[1], a, a, a, b[1])
        sub(res[2], a, a, a, b[2])
        return res
    ...
    fun sub(res: Mat3x3, a: Mat3x3, b: Mat3x3): Mat3x3 {
        sub(res[0], a[0], b[0][0], b[0][1], b[0][2])
        sub(res[1], a[1], b[1][0], b[1][1], b[1][2])
        sub(res[2], a[2], b[2][0], b[2][1], b[2][2])
        return res
    }


    fun mul(res: Mat3x3, a: Mat3x3, b: Float): Mat3x3 {
        mul(res[0], a[0], b, b, b)
        mul(res[1], a[1], b, b, b)
        mul(res[2], a[2], b, b, b)
        return res
    ...
    fun mul(res: Vec3, a: Mat3x3, b: Vec3): Vec3 {
    ...
    fun mul(res: Vec3, a: Vec3, b: Mat3x3): Vec3 {
    ...
    fun mul(res: Mat3x3, a: Mat3x3, b: Mat3x3): Mat3x3 {
    ...
    fun mul(res: Mat2x3, a: Mat3x3, b: Mat2x3): Mat2x3 {
        val v00 = a[0][0] * b[0][0] + a[1][0] * b[0][1] + a[2][0] * b[0][2]
        val v01 = a[0][1] * b[0][0] + a[1][1] * b[0][1] + a[2][1] * b[0][2]
        val v02 = a[0][2] * b[0][0] + a[1][2] * b[0][1] + a[2][2] * b[0][2]
        val v10 = a[0][0] * b[1][0] + a[1][0] * b[1][1] + a[2][0] * b[1][2]
    ...
    fun mul(res: Mat4x3, a: Mat3x3, b: Mat4x3): Mat4x3 {
        val v00 = a[0][0] * b[0][0] + a[1][0] * b[0][1] + a[2][0] * b[0][2]
        val v01 = a[0][1] * b[0][0] + a[1][1] * b[0][1] + a[2][1] * b[0][2]
        val v02 = a[0][2] * b[0][0] + a[1][2] * b[0][1] + a[2][2] * b[0][2]
        val v10 = a[0][0] * b[1][0] + a[1][0] * b[1][1] + a[2][0] * b[1][2]
    ...
    fun div(res: Mat3x3, a: Mat3x3, b: Mat3x3) = b.inverse(res) mul_ a
}


// -- Specific binary arithmetic operators --

operator fun Float.plus(b: Mat3x3) = add(Mat3x3(), b, this)
fun Float.add(b: Mat3x3, res: Mat3x3 = Mat3x3()) = add(res, b, this)
infix fun Float.add_(b: Mat3x3) = add(b, b, this)

operator fun Float.minus(b: Mat3x3) = sub(Mat3x3(), this, b)
fun Float.sub(b: Mat3x3, res: Mat3x3 = Mat3x3()) = sub(res, this, b)
infix fun Float.sub_(b: Mat3x3) = sub(b, this, b)

operator fun Float.times(b: Mat3x3) = mul(Mat3x3(), b, this)
fun Float.mul(b: Mat3x3, res: Mat3x3 = Mat3x3()) = mul(res, b, this)
infix fun Float.mul_(b: Mat3x3) = mul(b, b, this)

operator fun Vec3.times(b: Mat3x3) = mul(Vec3(), this, b)
fun Vec3.mul(b: Mat3x3, res: Vec3 = Vec3()) = mul(res, this, b)
infix fun Vec3.mul_(b: Mat3x3) = mul(this, this, b)
