...
import  mat.Mat3x4
import  mat.Mat4x4
import  mat.Mat4x4.Companion.add
import  mat.Mat4x4.Companion.div
import  mat.Mat4x4.Companion.mul
import  mat.Mat4x4.Companion.sub
import  vec._4.Vec4
...
interface mat4x4_operators {
    ...
    fun add(res: Mat4x4, a: Mat4x4, b: Float): Mat4x4 {
        add(res[0], a[0], b, b, b, b)
        add(res[1], a[1], b, b, b, b)
        add(res[2], a[2], b, b, b, b)
        add(res[3], a[3], b, b, b, b)
        return res
    }

    fun add(res: Mat4x4, a: Mat4x4, b: Mat4x4): Mat4x4 {
        add(res[0], a[0], b[0][0], b[0][1], b[0][2], b[0][3])
        add(res[1], a[1], b[1][0], b[1][1], b[1][2], b[1][3])
        add(res[2], a[2], b[2][0], b[2][1], b[2][2], b[2][3])
        add(res[3], a[3], b[3][0], b[3][1], b[3][2], b[3][3])
        return res
    }

    fun sub(res: Mat4x4, a: Mat4x4, b: Float): Mat4x4 {
        sub(res[0], a[0], b, b, b, b)
        sub(res[1], a[1], b, b, b, b)
        sub(res[2], a[2], b, b, b, b)
        sub(res[3], a[3], b, b, b, b)
        return res
    }

    fun sub(res: Mat4x4, a: Float, b: Mat4x4): Mat4x4 {
        sub(res[0], a, a, a, a, b[0])
        sub(res[1], a, a, a, a, b[1])
        sub(res[2], a, a, a, a, b[2])
        sub(res[3], a, a, a, a, b[3])
        return res
    }

    fun sub(res: Mat4x4, a: Mat4x4, b: Mat4x4): Mat4x4 {
        sub(res[0], a[0], b[0][0], b[0][1], b[0][2], b[0][3])
        sub(res[1], a[1], b[1][0], b[1][1], b[1][2], b[1][3])
        sub(res[2], a[2], b[2][0], b[2][1], b[2][2], b[2][3])
        sub(res[3], a[3], b[3][0], b[3][1], b[3][2], b[3][3])
        return res
    }
     ...
     * [res] = [a] * [b]
     */
    fun mul(res: Mat4x4, a: Mat4x4, b: Float): Mat4x4 {
        mul(res[0], a[0], b, b, b, b)
        mul(res[1], a[1], b, b, b, b)
        mul(res[2], a[2], b, b, b, b)
        mul(res[3], a[3], b, b, b, b)
        return res
    }

    fun mul(res: Vec4, a: Mat4x4, b: Vec4): Vec4 {
    ...
    fun mul(res: Vec4, a: Vec4, b: Mat4x4): Vec4 {
    ...
    fun mul(res: Mat2x4, a: Mat4x4, b: Mat2x4): Mat2x4 {
    ...
    fun mul(res: Mat3x4, a: Mat4x4, b: Mat3x4): Mat3x4 {
        val v00 = a[0][0] * b[0][0] + a[1][0] * b[0][1] + a[2][0] * b[0][2] + a[3][0] * b[0][3]
        val v01 = a[0][1] * b[0][0] + a[1][1] * b[0][1] + a[2][1] * b[0][2] + a[3][1] * b[0][3]
        val v02 = a[0][2] * b[0][0] + a[1][2] * b[0][1] + a[2][2] * b[0][2] + a[3][2] * b[0][3]
        val v03 = a[0][3] * b[0][0] + a[1][3] * b[0][1] + a[2][3] * b[0][2] + a[3][3] * b[0][3]
    ...
    fun mul(res: Mat4x4, a: Mat4x4, b: Mat4x4): Mat4x4 {
        val v00 = a[0][0] * b[0][0] + a[1][0] * b[0][1] + a[2][0] * b[0][2] + a[3][0] * b[0][3]
        val v01 = a[0][1] * b[0][0] + a[1][1] * b[0][1] + a[2][1] * b[0][2] + a[3][1] * b[0][3]
        val v02 = a[0][2] * b[0][0] + a[1][2] * b[0][1] + a[2][2] * b[0][2] + a[3][2] * b[0][3]
        val v03 = a[0][3] * b[0][0] + a[1][3] * b[0][1] + a[2][3] * b[0][2] + a[3][3] * b[0][3]
    ...
    fun div(res: Mat4x4, a: Mat4x4, b: Mat4x4) = b.inverse(res) mul_ a
}


// -- Specific binary arithmetic operators --

operator fun Float.plus(b: Mat4x4) = add(Mat4x4(), b, this)
fun Float.add(b: Mat4x4, res: Mat4x4 = Mat4x4()) = add(res, b, this)
infix fun Float.add_(b: Mat4x4) = add(b, b, this)

operator fun Float.minus(b: Mat4x4) = sub(Mat4x4(), this, b)
fun Float.sub(b: Mat4x4, res: Mat4x4 = Mat4x4()) = sub(res, this, b)
infix fun Float.sub_(b: Mat4x4) = sub(b, this, b)

operator fun Float.times(b: Mat4x4) = mul(Mat4x4(), b, this)
fun Float.mul(b: Mat4x4, res: Mat4x4 = Mat4x4()) = mul(res, b, this)
infix fun Float.mul_(b: Mat4x4) = mul(b, b, this)

operator fun Vec4.times(b: Mat4x4) = mul(Vec4(), this, b)
fun Vec4.mul(b: Mat4x4, res: Vec4 = Vec4()) = mul(res, this, b)
infix fun Vec4.mul_(b: Mat4x4) = mul(this, this, b)
