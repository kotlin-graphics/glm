...
@KotlinPoetDsl
inline fun TypeSpec.Builder.property(mutable: Boolean = false, vararg modifiers: KModifier, block: PropertyBuilder.() -> Unit) {
    val helper = PropertyBuilder()
    helper.block()
    val builder = PropertySpec.builder(helper.name, helper.type, *modifiers)
            .mutable(mutable)
    helper.initializer?.let(builder::initializer)
    addProperty(builder.build())
}

class PropertyBuilder {
    val modifiers = arrayListOf<KModifier>()
