    ...
    for (i in 2..4)
        for (j in 2..4)
            generate(target, "glm_/mat${matrixSize(i, j)}/Mat${matrixSize(i, j)}T.kt") {
                Generator.Companion.width = i
                Generator.Companion.height = j
                matricesT(i, j)
            }
...
val Generator.matrixSize: String
    get() {
...
private fun Generator.matricesT(width: Int, height: Int) {
    ...
    +"import glm_.vec$height.*"
    +"import glm_.vec$width.*"
    //    abcd(3, 3) { c, r, s -> +"import glm.mat${matrixSizeString(c + 2, r + 2)}.*" }
    for (i in 2..4) {
        +"import glm_.mat${matrixSize(i, width)}.*"
        +"import glm_.mat${matrixSize(i, height)}.*"
    }
...
private fun Generator.matrices(width: Int, height: Int, type: String, extension: String, id: String) {
    ...
    "open class $matID protected constructor(var array: ${type}Array) : $MatNT<$type, Vec$height$id>()" {
        ...
        +"// -- Constructors --"
        +"constructor() : this(1)"
        +"constructor(m: $matID) : this(${abcdJoint(",\n") { "m.$it" }})"
        ...
        +"constructor(m: $MatNT<*, Vec${height}T<*>>) : this(${abcdJoint(",\n") { "m.$it" }})"

        if (width == height) {
            if (width > 2) {
                val `xyzw Number` = xyzwJoint(width - 1) { c -> "$c: Number" }
                +"constructor($`xyzw Number`) : this(${xyzwJoint(width - 1) { c -> c }}, 1)"
            }
            +"constructor(${xyzwJoint(width) { c -> "$c: Number" }}) : this($xyzwDiag)"
        }
        ...
        abcdIndexed(3, 3) { i, j, _ ->
            ...
            if (c != width || r != height) {
                val sep = ",\n" + if (c != r) "  " else ""
                val args = abcdJointIndexed(sep) { col, row, text ->
                    when {
                        row < r && col < c -> "m.$text"
                        col == row -> "1"
                        else -> "0"
        ...
        if (width > 2)
            +"operator fun invoke(${xyzwJoint(width - 1) { "$it: Number" }}): $matID = invoke(${xyzwJoint(width - 1) { it }}, 1)"
        ...
        if (width > 2)
            +"fun put(${xyzwJoint(width - 1) { "$it: Number" }}) = put(${xyzwJoint(width - 1) { it }}, 1)"

        +"fun put(${xyzwJoint(width) { c -> "$c: Number" }}) = put($xyzwDiag)"
        ...
        if (type in floatingPointTypes && (matrixSize == "3" || matrixSize == "4")) {
            val ext = if (type == "Float") "" else "d"
            docs("Converts a pure rotation 3 * 3 matrix to a quaternion.")
