        ...
        when (part) {
            ...
            Generator.Part.CompanionObject -> {
                ...
                "inline fun <R> $func($`mAbcdN type`,\n$`nAbcdN type`, res: ($`xyzw Boolean`) -> R): R" {
                    +contract
                    fun equal(c: Int) {
                        val mC = (0 until height).joinToString { "m${glm_.ABCD[c]}$it" }
                        val nC = (0 until height).joinToString { "n${glm_.ABCD[c]}$it" }
                        val abcdCN = (0 until height).joinToString { glm_.abcdN(c, it) }
                        val `mAbcdN func nAbcdN` = AbcdJointIndexed(columnSeparator = " $sep ") { c, r, _ -> "${glm_.abcdN(c, r)}" }
